...
import { Activity, Check, Loader2, Lock, Plus, RefreshCw, Search, ShieldAlert, Wrench } from 'lucide-react';
...
export function SecurityScreen() {
  ...
  return <Page title="Security Center" note="Scoped agent execution keys, integrity state, and emergency controls." actions={<Button variant={status?.lockdown ? 'secondary' : 'danger'} onClick={() => lockdown(!status?.lockdown)}><Lock size={14}/>{status?.lockdown ? 'Unlock ToolGate' : 'Enable lockdown'}</Button>}>
    {status?.lockdown && <div className="mb-5 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200"><strong>Lockdown is active.</strong> Agent execution is blocked.</div>}
